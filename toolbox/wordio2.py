...
class WordLayoutReader(xml.sax.handler.ContentHandler):
	...
	def startElement(self, name, attrs):
		...
		if name == "TextLine":
			text_line = TextLine()
			text_line.top = int(attrs['top'])
			text_line.bottom = int(attrs['bottom'])
			text_line.left = int(attrs['left'])
			text_line.right = int(attrs['right'])
			text_line.shear = int(attrs['shear'])
			self.cur_line = text_line
		...
		if name == "Character":
			...
			self.cur_line.words[-1].characters.append(char)
